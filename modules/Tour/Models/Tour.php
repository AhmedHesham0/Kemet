                ...
                if (!empty($meta->person_types)) {
                    ...
                    foreach ($meta->person_types as $k => $type) {
                        if (isset($person_types_input[$k]) and $person_types_input[$k]['number']) {
                            ...
                            $person_types_provided = true;
                        }
                    }
                    // If no person types data was provided, fall back to regular guests
                    if (!$person_types_provided && $request->input('guests')) {
    ...
    public function addToCartValidate(Request $request)
    {
        ...
        if ($meta) {
            ...
            if ($meta->enable_person_types) {
                // Only unset guests requirement if person_types data is actually provided
                $person_types_input = $request->input('person_types');
                $has_person_types_data = false;
                if (!empty($person_types_input) && is_array($person_types_input)) {
                    foreach ($person_types_input as $type_data) {
                        if (!empty($type_data['number'])) {
                            $has_person_types_data = true;
                            break;
                        }
                    }
                }
                
                if ($has_person_types_data) {
                    unset($rules['guests']);
                }
                
                // Remove the person_types requirement to allow booking without it
