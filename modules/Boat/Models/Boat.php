...
class Boat extends Bookable
{
    ...
    public function addToCartValidate(Request $request)
    {
        ...
        // Bypass availability validation - allow booking on any date
        // Set all required temporary variables for the booking process
        if($type == 'per_hour'){
            $this->tmp_price = $this->price_per_hour * $hour;
        } else {
            $this->tmp_price = $this->price_per_day * $day;
        }
        
        // Set all required temporary variables
        $this->tmp_start_date = $start_date_time;
        $this->tmp_end_date = $end_date_time;
        $this->tmp_type_date = $type;
        
        // Create basic dates array for the booking
        $allDates = [];
        if($type == 'per_hour'){
            $allDates[$start_date] = [
                'number'=>$this->number ?? 1,
                'price'=>$this->tmp_price,
                'status'=>1
            ];
        } else {
