    ...
    <form action="{{route('hotel.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    <h1 class="title-bar">{{$row->id ? __('Edit: ').$row->title : __('Add new tour guide')}}</h1>
                    @if($row->slug)
                ...
                <div class="">
                    @if($row->id)
                        <a class="btn btn-warning btn-xs" href="{{route('hotel.admin.room.index',['hotel_id'=>$row->id])}}" target="_blank"><i class="fa fa-hand-o-right"></i> {{__("Manage Tour Guide")}}</a>
                    @endif
                    @if($row->slug)
                        <a class="btn btn-primary btn-xs" href="{{$row->getDetailUrl(request()->query('lang'))}}" target="_blank">{{__("View Tour Guide")}}</a>
                    @endif
                </div>
            </div>
                    ...
                    <div class="col-md-3">
                        ...
                        @if(is_default_lang())
                            <div class="panel">
                                ...
                                <div class="panel-body">
                                    <div class="form-group">
                                        <label>{{__('Tour Guide Featured')}}</label>
                                        <br>
                                        <label>
