...
class Space extends Bookable
{
    ...
    {
        $meta['seo_title'] = __("Search for Culture");
    ...
    public function addToCartValidate(Request $request)
    {
        ...
        if($total_guests > $max_guests){
            return $this->sendError(__("Maximum guests is :count",['count'=>$max_guests]));
        }
    ...
    public static function getReviewStats()
    {
        ...
        if (!empty($list = setting_item("space_review_stats", []))) {
            ...
            foreach ($list as $item) {
                // Skip Sleep and Rooms from review stats
                if (in_array(strtolower($item['title']), ['sleep', 'rooms'])) {
                    continue;
                }
