        ...
        <div class="bravo-list-item">
            <div class="d-flex justify-content-between align-items-center mb-4 topbar-search">
                <h3 class="font-size-21 font-weight-bold mb-0 text-lh-1">
                    @if($rows->total() > 1)
                        {{ __(":count Tour Guides found",['count'=>$rows->total()]) }}
                    @else
                        {{ __(":count Tour Guide found",['count'=>$rows->total()]) }}
                    @endif
                </h3>
                <div class="control">
            ...
            <div class="list-item">
                    @if($rows->total() > 0)
                        ...
                        @endforeach
                    @else
                            {{__("Tour Guide not found")}}
                    @endif
            </div>
            @if($rows->total() > 0)
                <div class="text-center text-md-left font-size-14 mb-3 text-lh-1">{{ __("Showing :from - :to of :total Tour Guides",["from"=>$rows->firstItem(),"to"=>$rows->lastItem(),"total"=>$rows->total()]) }}</div>
            @endif
